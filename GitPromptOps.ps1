...
# ==========================================
$script:Config = $null
$script:ConfigFile = Join-Path (Split-Path $MyInvocation.PSCommandPath) "gitprompt-config.json"
...
function Export-GitPromptConfiguration {
    ...
    $gitignore = Join-Path (Split-Path $MyInvocation.PSCommandPath) ".gitignore"
...
function Initialize-GitPromptSetup {
    ...
    switch ($choice) {
        '1' {
            ...
            $repos = Get-ChildItem -Path $env:USERPROFILE -Directory -Recurse -Depth 3 -Filter ".git" -ErrorAction SilentlyContinue | Select-Object @{N='Path';E={$_.Parent.FullName}}
            if ($repos) {
    ...
    if (-not $repoPath) { Write-Error "Setup failed."; exit }
