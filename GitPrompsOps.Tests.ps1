...
"@
  ...
  It 'Get-PromptsInventory skips README and invalid file names' {
    ...
    $valid = Join-Path $repoRoot 'prompts\\ops\\OPS__inv__v0.1.0.md'
    $readme = Join-Path $repoRoot 'prompts\\ops\\README.md'
    $invalid = Join-Path $repoRoot 'prompts\\ops\\OPS_bad__v0.1.0.md'
    Set-Content -Path $valid -Value 'ok' -Encoding UTF8
  ...
  It 'Resolve-TargetPromptPath returns selected prompt when multiple versions' {
    $origSelect = (Get-Command Select-PromptInteractive).ScriptBlock
    $repoRoot = $script:Config.RepoPath
    $inv = @(
      [pscustomobject]@{ Domain='ops'; Slug='pickme'; Version=[version]'0.1.0'; Path='p1'; RelPath='p1'; SizeKB=1 },
      [pscustomobject]@{ Domain='ops'; Slug='pickme'; Version=[version]'0.2.0'; Path='p2'; RelPath='p2'; SizeKB=1 }
    )
    try {
      Set-Item -Path function:Select-PromptInteractive -Value { param($promptMatches) $promptMatches | Sort-Object Version -Descending | Select-Object -First 1 } -Force
      $res = Resolve-TargetPromptPath -repoRoot $repoRoot -inv $inv -domain 'ops' -slug 'pickme' -explicitPath $null
      $res.Target | Should -Be 'p2'
    } finally {
      Set-Item -Path function:Select-PromptInteractive -Value $origSelect -Force
    }
  }
}
...
Describe 'GitPromptOps misc utilities' {
  ...
  It 'Read-YesNo returns true for y' {
    function Read-Host { param([string]$p) 'y' }
    try {
      Read-YesNo 'test' | Should -BeTrue
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'Read-YesNo returns false for n' {
    function Read-Host { param([string]$p) 'n' }
    try {
      Read-YesNo 'test' | Should -BeFalse
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'Read-TextFromPaste returns lines until END' {
    $script:rhLines = @('line1','line2','END')
    function Read-Host {
      param([string]$p)
      $value = $script:rhLines[0]
      if ($script:rhLines.Count -gt 1) {
        $script:rhLines = $script:rhLines[1..($script:rhLines.Count-1)]
      } else {
        $script:rhLines = @()
      }
      return $value
    ...
    try {
      Read-TextFromPaste 'test' | Should -Be "line1`nline2"
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'New-PromptLabPack creates a zip pack' {
    $repoRoot = $script:Config.RepoPath
    $zip = $null
    try {
      $zip = New-PromptLabPack -repoRoot $repoRoot
      $zip | Should -Match 'promptlab_upload_'
  ...
  It 'Invoke-Git throws when Git is not installed' {
    $origTest = (Get-Command Test-GitInstalled).ScriptBlock
    Set-Item -Path function:Test-GitInstalled -Value { $false } -Force
    try {
      { Invoke-Git -repoRoot $script:Config.RepoPath -gitArgs 'status' } | Should -Throw
    } finally {
      Set-Item -Path function:Test-GitInstalled -Value $origTest -Force
    }
  }
...
Describe 'GitPromptOps config and setup (mocked)' {
  BeforeAll {
    ...
    $script:baseConfig = $script:Config
    $script:baseConfigFile = $script:ConfigFile
  }

  AfterEach {
    $script:Config = $script:baseConfig
    $script:ConfigFile = $script:baseConfigFile
  }
  ...
  It 'Initialize-GitPromptSetup creates config for local folder (option 3)' {
    ...
    $createdRepo = Join-Path (Split-Path $script:scriptPath -Parent) 'testrepo'
    function Read-Host {
      param([string]$p)
...
Describe 'GitPromptOps git setup (mocked)' {
  ...
  It 'Test-GitInstalled returns true when git responds' {
    $orig = Get-Command git -ErrorAction SilentlyContinue
    $hadFunction = $orig -and $orig.CommandType -eq 'Function'
    function git { param([Parameter(ValueFromRemainingArguments=$true)][string[]]$args) 'git version 2.0.0' }
    try {
      Test-GitInstalled | Should -BeTrue
    } finally {
      if ($hadFunction) {
        Set-Item -Path function:git -Value $orig.ScriptBlock -Force
      } else {
  ...
  It 'Test-GitDoctor returns without throwing when Git is missing' {
    $origTest = (Get-Command Test-GitInstalled).ScriptBlock
    Set-Item -Path function:Test-GitInstalled -Value { $false } -Force
    try {
      { Test-GitDoctor -repoRoot $script:Config.RepoPath } | Should -Not -Throw
    } finally {
      Set-Item -Path function:Test-GitInstalled -Value $origTest -Force
    }
  }
...
Describe 'GitPromptOps menu flows (mocked)' {
  ...
  It 'Select-PromptInteractive throws on invalid selection' {
    $matches = @(
      [pscustomobject]@{ RelPath='a'; Version=[version]'0.1.0'; SizeKB=1 }
    )
    function Read-Host { param([string]$p) 'x' }
    try {
      { Select-PromptInteractive $matches } | Should -Throw
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }
...
Describe 'GitPromptOps remaining utilities' {
  BeforeAll {
  ...
  It 'Enable-SystemProxyForSession does not throw' {
    { Enable-SystemProxyForSession } | Should -Not -Throw
  }

  It 'Get-RepoRoot finds nearest repo' {
    $root = Join-Path $TestDrive 'root'
    $child = Join-Path $root 'a\\b'
    New-Item -ItemType Directory -Path $child -Force | Out-Null
    New-Item -ItemType Directory -Path (Join-Path $root '.git') -Force | Out-Null
    Push-Location $child
    try {
      Get-RepoRoot | Should -Be $root
    } finally {
      Pop-Location
    }
  }

  It 'Initialize-DomainFolder creates prompts structure' {
    $repoRoot = $script:Config.RepoPath
    Initialize-DomainFolder $repoRoot
    Test-Path (Join-Path $repoRoot 'prompts\\strategy') | Should -BeTrue
    Test-Path (Join-Path $repoRoot 'prompts\\coding') | Should -BeTrue
    Test-Path (Join-Path $repoRoot 'prompts\\ops') | Should -BeTrue
    Test-Path (Join-Path $repoRoot 'prompts\\trading') | Should -BeTrue
  }

  It 'Get-LatestPrompt returns highest version' {
    $inv = @(
      [pscustomobject]@{ Domain='ops'; Slug='x'; Version=[version]'0.1.0' },
      [pscustomobject]@{ Domain='ops'; Slug='x'; Version=[version]'0.2.0' }
    )
    (Get-LatestPrompt -inv $inv -domain 'ops' -slug 'x').Version.ToString() | Should -Be '0.2.0'
  }

  It 'Select-PromptInteractive returns selected item' {
    $matches = @(
      [pscustomobject]@{ RelPath='a'; Version=[version]'0.1.0'; SizeKB=1 },
      [pscustomobject]@{ RelPath='b'; Version=[version]'0.2.0'; SizeKB=2 }
    )
    function Read-Host { param([string]$p) '1' }
    try {
      (Select-PromptInteractive $matches).RelPath | Should -Be 'b'
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'Get-CommitMessage-Interactive uses defaults' {
    function Read-Host { param([string]$p) '' }
    try {
      Get-CommitMessage-Interactive -defaultScope 'ops/x' -defaultSubject 'update content' | Should -Match 'fix\(ops/x\): update content'
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'Invoke-Menu exits on choice 0' {
    $repoRoot = $script:Config.RepoPath
    function Read-Host { param([string]$p) '0' }
    try {
      { Invoke-Menu $repoRoot } | Should -Not -Throw
    } finally {
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
    }
  }

  It 'Invoke-AutoSetup throws when user declines clone' {
    $orig = (Get-Command Initialize-GitEnvironment).ScriptBlock
    Set-Item -Path function:Initialize-GitEnvironment -Value { } -Force
    function Test-GitHubConnectivity { $true }
    function Read-Host { param([string]$p) 'N' }
    try {
      { Invoke-AutoSetup -targetDir $script:Config.RepoPath } | Should -Throw
    } finally {
      Set-Item -Path function:Initialize-GitEnvironment -Value $orig -Force
      Remove-Item -Path function:Read-Host -ErrorAction SilentlyContinue
      Remove-Item -Path function:Test-GitHubConnectivity -ErrorAction SilentlyContinue
    }
  }

  It 'Update-GitPromptScript throws when script missing' {
    $repoRoot = $script:Config.RepoPath
    $orig = $PSCommandPath
    $script:PSCommandPath = Join-Path $repoRoot 'missing.ps1'
    try {
